...
void DFS(int x, int y)
{
	if (CanDFS(x, y))
	{
		if (max_result.length == total_int_cnt)
		{
			for (int i = 0; i < temp_result.length; ++i)
			{
				...
				else if (temp_result.route[i] == max_result.route[i])
				{
					continue;
				}
				else
...
int BFS(int x, int y)
{
	...
	while (que_size > 0)
	{
		...
		if (CanBFS(new_x, new_y))
		{
			...
			{
				my_queue[++cur_indx] = ((new_x - 1) * RECT_SIZE + new_y);
				++que_size;
			}

			if (CanBFS(new_x, new_y - 1))
			{
				my_queue[++cur_indx] = (new_x * RECT_SIZE + (new_y - 1));
				++que_size;
			}
